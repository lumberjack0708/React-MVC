<?php
    ...
    class Product{
        ...
        public function newProduct($p_name, $cost, $price, $stock){
            // 檢查必填欄位
            if(empty($p_name)) {
            ...
            // 檢查產品名稱是否已存在
            $checkSql = "SELECT COUNT(*) AS count FROM `product` WHERE `p_name` = ?";
        ...
        public function updateProduct($pid, $p_name, $cost, $price, $stock){
            // 檢查必填欄位
            if(empty($p_name)) {
                return array('status' => 400, 'message' => "產品名稱不可為空");
            }
            ...
            // 檢查產品名稱是否已存在且不是當前產品
            $checkSql = "SELECT COUNT(*) AS count FROM `product` WHERE `p_name` = ? AND `pid` != ?";
